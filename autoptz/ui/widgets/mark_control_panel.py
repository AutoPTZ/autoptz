...
class MarkControlPanel(QWidget):
    ...
    def _restyle_buttons(self) -> None:
        ...
        clear secondary action: a bigger accent-bordered button in the active palette
        so the two Mark-level controls read as one grouped control cluster.
        """
        ...
        self._exit_btn.setStyleSheet(
            ...
            f" color: {T.ACCENT.name()}; border: 1px solid {T.ACCENT.name()};"
            ...
            f" color: {T.ACCENT.lighter(115).name()};"
            f" border-color: {T.ACCENT.lighter(115).name()}; }}"
        )
